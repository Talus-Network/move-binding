...
/// Whether a field is the `id: UID` package field required by `key` objects.
///
/// `UID` is intentionally recognized structurally, by field name and final path segment, because
/// the framework declaration is not a core `talus-sui-move` type. Generated framework bindings
/// and local package fixtures should therefore satisfy the default rule without coupling this
/// derive crate to a handwritten core UID mirror.
