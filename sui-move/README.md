...
- verify type tags returned from chain state and decode BCS in a controlled way.
...
`MoveStruct` is the specialization for struct types that produces a `StructTag`.
...
If you prefer, enable the `derive` feature to use attribute macros for defining Rust structs that
represent Move structs.
...
### Decoding with tag verification (`MoveInstance`)
...
`talus-sui-move` intentionally does not export handwritten mirrors for Sui framework packages such as
`0x2::object::UID`, `0x2::coin::Coin<T>`, `0x2::balance::Balance<T>`, or `0x1::option::Option<T>`.
Those are datatypes defined by a package, not language atoms.
...
- `talus_sui_move::prelude`: convenient imports for common traits/types
- `talus_sui_move::decode`: decoding helpers constrained by Move abilities
