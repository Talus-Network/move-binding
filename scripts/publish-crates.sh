...
if ! git merge-base --is-ancestor "$tag_commit" origin/main; then
    echo "$release_tag must point to a commit on origin/main." >&2
    exit 1
fi
...
api_resource_exists() {
    ...
    http_status="$(curl \
        ...
        --output /dev/null \
        --write-out '%{http_code}' \
        --header 'User-Agent: Talus move-binding release (https://github.com/Talus-Network/move-binding)' \
...
existing_package_count=0
for package in "${packages[@]}"; do
    if api_resource_exists "$package"; then
        existing_package_count=$((existing_package_count + 1))
    elif [[ "$?" != "1" ]]; then
        exit 1
    fi
done

if (( existing_package_count > 0 && existing_package_count < ${#packages[@]} )); then
    if [[ "$allow_existing" == "0" ]]; then
        echo "Only $existing_package_count of ${#packages[@]} package names exist on crates.io." >&2
        echo "Investigate the owners, or use ALLOW_EXISTING=1 for reviewed recovery." >&2
        exit 1
    fi
fi
...
echo "Published all move-binding packages for $release_tag."
